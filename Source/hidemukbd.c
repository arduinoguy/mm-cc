...
uint8 report[8]={0,0,0,0,0,0,0,0};
uint8 report_index=2;
uint8 stroke =0x04;
...
static CONST uint8 attDeviceName[GAP_DEVICE_NAME_LEN] = "MAKEY";
...
static void hidEmuKbdSendReports(uint8,uint8,uint8,uint8,uint8,uint8,uint8);
static void hidEmuKbd_ProcessOSALMsg( osal_event_hdr_t *pMsg );
static void hidEmuKbd_HandleKeys( uint8 shift, uint8 keys );
static void hidEmuKbdSendReport( uint8 dummy[8]);
...
void key_press(uint8);
void key_release(uint8);
void release_all();
void serial_setup();
 ...
 *          This is called during ization and should contain
...
{  
 ...
 /* if((((keyChange_t *)pMsg)->keys)=='#')
    key_press(((keyChange_t *)pMsg)->state);
  
  else key_release(((keyChange_t *)pMsg)->state);*/
  
   key_press(((keyChange_t *)pMsg)->state);
   asm("nop");
   key_release(((keyChange_t *)pMsg)->state);
   asm("nop");
...

{
  ...
  buf[0] = mod;         // Modifier keys
  buf[1] = 0;         // Reserved
  buf[2] = keycode1;   // Keycode 1
  buf[3] = keycode2;         // Keycode 2
  buf[4] = keycode3;         // Keycode 3
  buf[5] = keycode4;         // Keycode 4
  buf[6] = keycode5;         // Keycode 5
  buf[7] = keycode6;         // Keycode 6*/

  HidDev_Report( HID_RPT_ID_KEY_IN, HID_REPORT_TYPE_INPUT,
                HID_KEYBOARD_IN_RPT_LEN, dummy );
}
...
static uint8 hidEmuKbdRptCB( uint8 id, uint8 type, uint16 uuid,uint8 oper, uint8 *pLen, uint8 *pData )
{
...
__interrupt void p0_ISR(void)
{
  ...
  P0IF=0;
  P0_1^=1;
  OnBoard_SendKeys(0,0);
}

void interrupt_setup()
{
   P0SEL &= ~(BIT0|BIT1); //general purpose i/o
   P0DIR &= ~BIT0; 
   P0DIR|=BIT1;
   P0INP&= ~BIT0; 
   P0_1=0;
...
__interrupt void uart0_rx_isr(void)
{
  ...
  else 
  {
    key_hex=U0DBUF;
    uart_flag=false;
    OnBoard_SendKeys(msg,key_hex);
  }*/
  key_hex=U0DBUF;
  OnBoard_SendKeys(0,key_hex);
...
void key_release(uint8 key)
{
  for(uint8 temp_index=2;temp_index<report_index;temp_index++)
  {
   if(report[temp_index]==key)
   {
     report[temp_index]=0;
     report_index=report_index-1;
     
     while(temp_index<report_index)
     {
       report[temp_index]=report[temp_index+1];
 ...
 //  else temp_index=temp_index+1;
  }
           
}

static void hidEmuKbdSendReports(uint8 mod,uint8 keycode1,uint8 keycode2,uint8 keycode3,uint8 keycode4,uint8 keycode5,uint8 keycode6)
